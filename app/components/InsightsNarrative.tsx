...
import type { InsightsNarrative, InsightsReportFile } from "@/app/lib/insights-narrative";
...
function ReportButton({ reportFile }: { reportFile: InsightsReportFile }) {
  ...
  return (
    <a href="/api/insights-report" target="_blank" rel="noreferrer" className={REPORT_BUTTON_CLASS}>
      Open Claude&apos;s full /insights report ↗
      <span className="opacity-70">· {date} · {kb} KB</span>
    </a>
  );
}

export default function InsightsNarrativeSection({ narrative, reportFile }: Props) {
    ...
    <section className="mb-16">
      ...
      {!narrative && reportFile && (
        <div className="bg-[color:var(--color-panel)] border border-dashed border-[color:var(--color-line)] rounded-xl p-5 text-sm text-[color:var(--color-mute)] leading-relaxed">
          Claude&apos;s full HTML report is on your disk from your last <span className="mono">/insights</span>{" "}
          run on {reportFile.capturedAt.slice(0, 10)}. Click the button above to open it. Optionally, you can
          also paste a markdown summary into <span className="mono">app/data/insights-narrative.md</span> for
      ...
      <p className="text-xs text-[color:var(--color-mute)] mt-3 leading-relaxed">
        {reportFile && (
          <>
            HTML report served locally from <span className="mono">~/.claude/usage-data/report.html</span> via{" "}
            <span className="mono">/api/insights-report</span>. Static file written by Claude Code; this dashboard
            doesn&apos;t generate or modify it.{" "}
          </>
        )}
        {narrative && (
          <>
            Inline narrative captured by you when you ran <span className="mono">/insights</span>; rendered from{" "}
            <span className="mono">app/data/insights-narrative.md</span> (gitignored). Not redistributed and not
            posted to Slack.{" "}
          </>
        )}
  ...
  return (
    <section className="mb-16">
      ...
      <div className="bg-[color:var(--color-panel)] border border-dashed border-[color:var(--color-line)] rounded-xl p-6 text-sm text-[color:var(--color-mute)] leading-relaxed space-y-3">
        <p>
          No <span className="mono">/insights</span> data found on disk yet. The dashboard scores your usage
          from raw signal files independently; if you also want Claude&apos;s own analysis surfaced here, run{" "}
...
pbpaste | npm run import-insights`}
        ...
        <p>
          Both artifacts stay on your machine. Nothing is uploaded, posted to Slack, or auto-captured.
        </p>
      </div>
    </section>
  );
