...
export interface DimensionExplainer {
  ...
  what: string;
  /** Links to deeper context (Boris tips already render via the rubric). */
  notes?: string;
}
 ...
 * (and ideally add a regression test that reads both).
 */
export const EXPLAINERS: Record<string, DimensionExplainer> = {
  automation: {
    ...
    formula: [
      { label: "Hooks configured", contributes: "+8 per configured hook event (PostToolUse, Stop, SessionStart, PostCompact, …)", max: 25 },
...
/**
 * Pick the highest-leverage move that would push the dimension's score up. We
 * lean on the rubric's `nextActions` (already ordered by author intent) and
 * prefer the lowest-effort prerequisite — small wins compound faster.
 */
export function plusTenPath(dim: Dimension): PlusTenPath | null {
  const ordered = [...dim.nextActions].sort((a, b) => effortMin(a.effort) - effortMin(b.effort));
  const best = ordered.find((a) => !a.requires?.length) ?? ordered[0];
  if (!best) return null;
  return {
    step: best.action,
    rationale: `Lowest-effort move (${best.effort}) that this dimension's rubric author flagged as load-bearing.`,
  };
}

function effortMin(e: string): number {
  return { "5min": 5, "15min": 15, "30min": 30, "1hr": 60, "2hr": 120 }[e] ?? 30;
}
